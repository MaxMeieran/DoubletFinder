doubletFinder_improve <- function(seu, PCs, pN = 0.25, pK, nExp, reuse.pANN = FALSE, sct = FALSE, annotations = NULL,assayused='RNA') {
  ...
  if (reuse.pANN == FALSE) {
    ...
    data <- seu@assays[[assayused]]@counts[, real.cells]
    ...
    if (sct == TRUE) {
      ...
      seu_wdoublets <- CellCycleScoring(seu_wdoublets , s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE) 

      seu_wdoublets <- SCTransform(seu_wdoublets,ncells =0.1*nrow(scRNA_harmony@meta.data),
